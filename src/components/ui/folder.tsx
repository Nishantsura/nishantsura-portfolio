...
const FolderCard: React.FC<FolderCardProps> = ({ title, className }) => {
  return (
    <section className={`relative group flex flex-col items-center justify-center w-full h-full ${className ?? ''}`}>
      ...
      {title && <p className="text-base mt-4 text-white/90">{title}</p>}
    </section>
  );
}
